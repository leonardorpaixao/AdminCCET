...
<div class="content-wrapper">
  ...
  <section class="content">
    ...
    <div class="box" id="box">
      <form action="post/" method="post" id="formulario" autocomplete="off" name="formulario" class="formulario">
        <input type="password" style="display:none">
        <input type="hidden" id="numPost" name="numPost" value="13"><!-- Número correspodente ao post -->
        <div class="box-body">
          <div class="box-body">
          ...
          <div class="form-group col-xs-6">
            <label for="email">Nivel de Acesso:</label>
            <select name="nivelAcesso" id="nivelAcesso1" class="form-control">
              <option value="">Selecione Nivel de Acesso</option>
              <option value="0">Administrador do Sistema</option>
              <option value="1">Gerênciador de Reserva</option>
              <option value="2">Basico</option>
            </select>
          </div>
          <div class="form-group col-xs-6">
            <label for="email">Afiliação:</label>
            <select name="idAfiliacao" id="idAfiliacao1" class="form-control" onchange="NovoTipo(this.value);">
              <?php
                echo '<option value="">Selecione Afiliação</option>';
                echo '<option value="0">Adicinar Nova Afiliação</option>';
                if($query = $db->prepare("SELECT idAfiliacao, afiliacao FROM tbAfiliacao WHERE nivel < 2")){
                  $query->execute();
        ...
        <div class="box-footer">
          ...
          <a href="/recursos/funcionarios"<span class="btn btn-default">Cancelar</span></a>
        </div>
      </form>
    </div><!-- /.box -->
  ...
  <script>
    $('#box').find('.formulario').submit(function() {
        ...
        var idAfiliacao = $.trim($(this).find('#idAfiliacao1').val());
        var nivelAcesso = $.trim($(this).find('#nivelAcesso1').val());
        var senha = $.trim($(this).find('#senha').val());
        var senha2 = $.trim($(this).find('#senha2').val());

        if(!(nome.length != 0 && cpf.length != 0 && email.length != 0
              && login.length != 0 && idAfiliacao != "" && nivelAcesso != "")) {
