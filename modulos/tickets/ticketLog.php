...
?>
  ...
  <div class="content-wrapper">
      ...
      <section class="content">
        <div class="box-body">
            ...
            if($total > 0):
              ...
              switch($statusTicket){
                ...
                case 'Em Analise':
                  ...
                  break;
                case 'Em Atendimento':
                  $status = '<span class="label label-warning">EM ATENDIMENTO</span>';
                  break;
                case 'Respondido':
              ...
              if($dono == $_SESSION['id']){
                ...
                if($statusTicket == 'Em Analise'){
                  $botao = '<button class="btn btn-success" data-toggle="modal" data-target="#simples2" data-solict-idticket="'.$idTicket.'" data-solict-tipo="5" data-solict-frase="Iniciar Atendimento">Iniciar Atendimento</button>';
                  if($idAssunto == 6){
                ...
                elseif($statusTicket == 'Em Atendimento'){
          ...
          <?php
            ...
            if(isset($statusTicket) && ($statusTicket == 'Em Analise' || $statusTicket == 'Em Atendimento')){
              $acao = 1;
            }else{
              $acao = 2;
            }
    ...
    <div class="modal fade" id="simples2" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span>
          ...
          <div class="modal-body">
            <input type="hidden" id="numPost" name="numPost" value="44"><!-- Número correspodente ao post -->
            <input type="hidden" name="idticket3" id="idticket3"/>
            <input type="hidden" name="acao3" id="acao3"/>
            <div class="form-group">
              <label>Clicando em "Confirmar" você deve iniciar o atendimento e só deve responder quando o mesmo for realizado.</label>
            </div>
          </div>
          <div class="modal-footer">
            <button type="button" class="btn btn-default" data-dismiss="modal">Cancelar</button>
...
<script>
  ...
  $('#simples2').on('show.bs.modal', function (event) {
    var button = $(event.relatedTarget) // Button that triggered the modal
    var tipo = button.data('solict-tipo')
    var frase = button.data('solict-frase')
    var id = button.data('solict-idticket')
