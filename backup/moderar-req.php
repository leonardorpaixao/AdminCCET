  ...
  <?php
    ...
    if($query = $db->prepare("SELECT a.idReq, a.conteudoReq, a.dataReq, a.statusReq, a.tipoReq, a.justificativaReq, b.matricula, c.nomeUser FROM tbrequerimentos a NATURAL LEFT JOIN tbreqs_professor d JOIN tbmatricula b ON a.idUser = b.idUser AND a.tipoReq != 5 AND ((d.idProfessor = 0) OR (d.idProfessor IS NULL) OR (a.statusReq = 'ConfirmadoProf') OR (a.statusReq = 'NegadoProf')) JOIN tbusuario c ON a.idUser = c.idUser UNION SELECT a.idReq, a.conteudoReq, a.dataReq, a.statusReq, a.tipoReq, a.justificativaReq, b.matricula, b.nome FROM tbrequerimentos a NATURAL LEFT JOIN tbreqs_professor d JOIN tbtemporarios b ON a.idTemp = b.idTemp AND a.tipoReq != 5 AND ((d.idProfessor = 0) OR (d.idProfessor IS NULL) OR (a.statusReq = 'ConfirmadoProf') OR (a.statusReq = 'NegadoProf'))")){
      $query->execute();
